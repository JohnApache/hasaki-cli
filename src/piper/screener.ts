...
const CreateScreener = (rootPath: string, screenRule: ScreenRule) => {
    ...
    const IsInclude = (targetPath: string): boolean => {
        return IsMatchRules(rootPath, targetPath, include);
    }

    const IsExclude = (targetPath: string): boolean => {
        return IsMatchRules(rootPath, targetPath, exclude);
    }

    const Screener = (targetFile: string, callback?: (targetDir: string) => void): void => {
